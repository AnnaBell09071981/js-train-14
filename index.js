...
function nextNewYear() {
  ...
  console.log(date, year1);
  date.setMonth(0);
  date.setDate(1);
  console.log(date.getFullYear() + 1, date.getMonth(0), date.getDate(1));
  const diff = date.getMilliseconds(year1);
  console.log(diff);
  ...
  console.log('day:', day, 'hours:', hours, 'minutes:', minutes, 'seconds:', seconds, 'milliseconds:', milliseconds);
  return date;
...
function addDays(date, days) {
  ...
  if(typeof date.getTime === "function") {
    if(typeof days === "number") {
      addedDays = days;
      resultDate = inputDate + addedDays;
      return {
          inputDate,
          addedDays,
          resultDate,
...
function getAge(birthDate) {
  ...
  let resultDate;
  if(typeof date.getTime === "function") {
    nowDateyear = inputDate.getFullYear();
    birthDateyear = birthDate.getFullYear();
    resultDate = nowDateyear - birthDateyear;
    nowDate = inputDate.getMonth();
    birdate = birthDate.getMonth();
    if(nowDate > birdate) {
      return {
        birthDate,
        resultDate,
...
function getDaysDifference(startDate, endDate) {
  ...
  let resultDate;
  if(typeof date.getTime === "function") {
    millisecondsSD = startDate.getMilliseconds();
    millisecondsED = endDate.getMilliseconds();
